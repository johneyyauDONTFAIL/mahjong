...
//		    deck.giveCard(true,in);
		    ...
		    while(deck.getRemainingCard()>0) {
				...
				String seq = deck.drop(p, index,in);
				...
				if(seq.equals("")){//someone action
					...
					System.out.println("-------------------------------------------------------------");
					System.out.println(String.format("Player %s WIN!!!",pList.get(Integer.parseInt(seq.split("%PARAM%")[1])).getName()));
					System.out.println("-------------------------------------------------------------");
					break;
				}
				else{
